...
public class RefProcessor {
    ...
    @Around("@annotation(er.rennala.advice.ref.Dereference) || @within(er.rennala.advice.ref.Dereference)")
    ...
    private void handleSingleReturn(Field fieldWithRefAnnotation,
                                    Object singleReturn) {
        fieldWithRefAnnotation.setAccessible(true);
        Ref ref = fieldWithRefAnnotation.getAnnotation(Ref.class);
        Dereferencer dereferencer = SpringUtil.getBean(ref.dereferencer());
        ...
        if (referencedFieldValues.size() == 1) {
            ...
            Object referencedObject = dereferencer.findOne(referencedFieldValues.get(0));
            if (Objects.nonNull(referencedObject)) {
                referencedObjects.add(referencedObject);
            }
        } else {
            referencedObjects = dereferencer.findMany(referencedFieldValues);
        }
        ...
        setReferencedObjectToSingleReturn(fieldWithRefAnnotation, singleReturn, referencedField2Object, ref, dereferencer);
    }

    private void handleCollectionReturn(Field fieldWithRefAnnotation,
                                        Collection<Object> collectionReturn) {
        fieldWithRefAnnotation.setAccessible(true);
        Ref ref = fieldWithRefAnnotation.getAnnotation(Ref.class);
        Dereferencer dereferencer = SpringUtil.getBean(ref.dereferencer());
        ...
        collectionReturn.forEach(r -> setReferencedObjectToSingleReturn(fieldWithRefAnnotation, r, referencedField2Object, ref, dereferencer));
    }

    /**
     * Get the value of spec field. e.g.
     */
    private List<Object> getFieldValueAsList(Field field,
                                             Object obj) {
     ...
     * @return map.
     */
    private Map<Object, Object> mappingReferencedObjects(Ref ref,
                                                         List<Object> referencedObjects) {
     ...
     * ref: @Ref(analyzer = UserRepositoryImpl::class, refField = "id", analysisTo = "players")
     * dereferencer: instance of UserRepositoryImpl
     *
     ...
     * @param ref                    the @Ref annotation.
     * @param dereferencer           the dereferencer
     */
    private void setReferencedObjectToSingleReturn(Field fieldWithRefAnnotation,
                                                   Object singleReturn,
                                                   Map<Object, Object> referencedField2Object,
                                                   Ref ref,
        ...
        try {
            ...
            // check the filedValue is null, if null, skip dereference
            ...
            if (Collection.class.isAssignableFrom(fieldWithRefAnnotation.getType())) {
                ...
                referencedObject = collectionFieldValue.stream().map(key -> {
                    ...
                    if (Objects.isNull(o)) {
                        o = dereferencer.fallback(key);
                    }
                    return dereferencer.convert(o);
                }).filter(Objects::nonNull).toList();
            } else {
                Object o = referencedField2Object.get(filedValue);
                if (Objects.isNull(o)) {
                    o = dereferencer.fallback(filedValue);
                }
                referencedObject = dereferencer.convert(o);
            }
            Field targetField = singleReturn.getClass().getDeclaredField(ref.dereferenceTo());
            ...
            log.error("[RNA-Ref] Get field failed. fieldName: {}", ref.dereferenceTo());
            throw new RuntimeException(e);
        }
    }
